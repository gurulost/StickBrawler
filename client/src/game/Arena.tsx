...
import { ARENA_THEMES, DEFAULT_ARENA, type ArenaTheme } from "./arenas";
...
const Arena = ({ variant = DEFAULT_ARENA }: ArenaProps) => {
  const theme = ARENA_THEMES[variant] || ARENA_THEMES[DEFAULT_ARENA];
  
  // Textures based on theme
  const gradientTexture = useMemo(
    () => createGradientTexture(theme.colors.floorTop, theme.colors.floorBottom),
    [theme.colors.floorTop, theme.colors.floorBottom],
  );
  const skylineTexture = useMemo(
    () => createGradientTexture(theme.colors.skyTop, theme.colors.skyBottom),
    [theme.colors.skyTop, theme.colors.skyBottom],
  );
  ...
  if (theme.style === 'contained') {
    return <ContainmentArena theme={theme} gradientTexture={gradientTexture} skylineTexture={skylineTexture} />;
  }
  
  // Render open arena (platform-based design)
  return <OpenArena theme={theme} gradientTexture={gradientTexture} skylineTexture={skylineTexture} />;
};

// Open Arena Component (Sunset Bloom, Aurora Flux)
const OpenArena = ({ theme, gradientTexture, skylineTexture }: { 
  theme: ArenaTheme; 
  ...
  const gridHelper = useMemo(() => {
    const helper = new THREE.GridHelper(ARENA_WIDTH, 30, theme.colors.gridColor1, theme.colors.gridColor2);
  ...
  return (
    <group>
        ...
        return (
          <group key={`platform-${index}`}>
            ...
            >
              ...
              <meshStandardMaterial color={theme.colors.platform} roughness={0.4} />
            </mesh>
            ...
            >
              ...
              <meshStandardMaterial color={theme.colors.platformSupport} />
            </mesh>
      ...
      <mesh position={[0, ARENA_WIDTH/2, -ARENA_WIDTH/2]}>
        ...
        <meshBasicMaterial map={skylineTexture ?? undefined} color={theme.colors.skyTop} />
      </mesh>
      
      {/* Boundaries */}
      <mesh position={[-ARENA_WIDTH/2, wallHeight/2, 0]} castShadow>
        <boxGeometry args={[1.2, wallHeight, ARENA_WIDTH/2]} />
        <meshStandardMaterial color={theme.colors.wallLeft} roughness={0.4} />
      </mesh>
      
      <mesh position={[ARENA_WIDTH/2, wallHeight/2, 0]} castShadow>
        <boxGeometry args={[1.2, wallHeight, ARENA_WIDTH/2]} />
        <meshStandardMaterial color={theme.colors.wallRight} roughness={0.7} />
      </mesh>
      ...
      {Array.from({ length: decorationCount }).map((_, i) => (
        ...
        >
          <mesh castShadow position={[0, 2, 0]}>
            ...
            <meshStandardMaterial emissive={theme.colors.decorEmissive1} color={theme.colors.decorBase1} />
          </mesh>
          <mesh castShadow position={[ARENA_WIDTH - 1, 2, 0]}>
            <torusGeometry args={[1, 0.08, 16, 64, Math.PI]} />
            <meshStandardMaterial emissive={theme.colors.decorEmissive2} color={theme.colors.decorBase2} />
          </mesh>
        </group>
      ))}
      ...
      <pointLight 
        intensity={theme.lighting.pointLight1Intensity} 
        position={[-ARENA_WIDTH/3, ARENA_WIDTH/5, ARENA_WIDTH/4]} 
        color={theme.lighting.pointLight1Color} 
      />
      <pointLight 
        intensity={theme.lighting.pointLight2Intensity}
        position={[ARENA_WIDTH/4, ARENA_WIDTH/3, -ARENA_WIDTH/4]}
        color={theme.lighting.pointLight2Color}
      />
    </group>
  );
};
...
const ContainmentArena = ({ theme, gradientTexture, skylineTexture }: {
  theme: ArenaTheme;
  gradientTexture: THREE.CanvasTexture | null;
  skylineTexture: THREE.CanvasTexture | null;
}) => {
  ...
  const polarGrid = useMemo(() => {
    const g = new THREE.PolarGridHelper(wallRadius * 1.02, 16, 8, 96, theme.colors.gridColor1, theme.colors.gridColor2);
  ...
  return (
    <group>
      ...
      <mesh>
        ...
        <meshBasicMaterial map={skylineTexture ?? undefined} side={THREE.BackSide} />
      </mesh>
      ...
      <mesh position={[0, FLOOR_Y + ringThickness / 2, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color={theme.colors.decorBase2} roughness={0.55} metalness={0.05} />
      </mesh>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, FLOOR_Y + ringThickness + 0.005, 0]}>
        <ringGeometry args={[ringRadius * 0.92, ringRadius * 0.98, 64]} />
        <meshBasicMaterial color={theme.colors.gridColor1} transparent opacity={0.55} />
      </mesh>
      ...
      <mesh position={[0, FLOOR_Y + wallHeight + 0.02, 0]}>
        ...
        <meshStandardMaterial color={theme.colors.wallRight} roughness={0.4} metalness={0.6} />
      </mesh>
      ...
      {railInstances.map(({ x, z, rotY, chord }, i) => (
        ...
        >
          ...
          <meshStandardMaterial color={theme.colors.wallLeft} transparent opacity={0.25} metalness={0.3} roughness={0.6} />
        </mesh>
      ))}
        ...
        return (
          <group key={`platform-${i}`}>
            <mesh position={[cx, y, cz]} castShadow receiveShadow>
              ...
              <meshStandardMaterial color={theme.colors.platform} roughness={0.45} metalness={0.05} />
            </mesh>
            <mesh rotation={[-Math.PI / 2, 0, 0]} position={[cx, y + 0.13, cz]}>
              <ringGeometry args={[haloR * 0.93, haloR, 48]} />
              <meshBasicMaterial color={theme.colors.decorEmissive2} transparent opacity={0.35} />
            </mesh>
          </group>
        );
      })}
      ...
      <pointLight 
        intensity={theme.lighting.pointLight1Intensity} 
        position={[-wallRadius * 0.6, wallRadius * 0.5, -wallRadius * 0.6]} 
        color={theme.lighting.pointLight1Color} 
      />
    </group>
  );
